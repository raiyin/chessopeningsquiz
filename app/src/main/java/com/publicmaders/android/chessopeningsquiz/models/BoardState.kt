...
object BoardState
{
    ...
    fun reset()
    {
        ...
        for (i in 0 until 2)
        {
            ...
            addPiece(ChessPiece(0,
                1 + i * 5,
                Player.WHITE,
                Chessman.KNIGHT,
                R.drawable.ic_chess_wn))
            addPiece(ChessPiece(7,
                1 + i * 5,
                Player.BLACK,
                Chessman.KNIGHT,
    ...
    fun getRookThatCanGoTo(square: Square, player: Player): MutableList<ChessPiece>
    {
        ...
        for (col in 0..7)
        {
            if (col != square.col)
            {
                ...
                if (tempPiece?.chessman == Chessman.ROOK && isRowCleanBetween(tempSquare,
                        square) && tempPiece.player == player)
                {
        ...
        for (row in 0..7)
        {
            if (row != square.row)
            {
                ...
                val tempPiece = getPieceFrom(tempSquare)
                if (tempPiece?.chessman == Chessman.ROOK && isColumnCleanBetween(tempSquare,
                        square) && tempPiece.player == player)
                {
                    resultList.add(tempPiece)
                }
    ...
    fun getBishopThatCanGoTo(square: Square, player: Player): MutableList<ChessPiece>
    {
        ...
        while (square.row + index < 8 && square.col + index < 8)
        {
            ...
            val tempPiece = getPieceFrom(tempSquare)
            if (tempPiece?.chessman == Chessman.BISHOP && tempPiece.player == player && isDiagonalCleanBetween(
                    tempSquare,
                    square))
            {
                resultList.add(tempPiece)
    ...
    fun getQueenThatCanGoTo(square: Square, player: Player): MutableList<ChessPiece>
    {
        ...
        for (col in 0..7)
        {
            if (col != square.col)
            {
                val tempSquare = Square(square.row, col)
                val tempPiece = getPieceFrom(tempSquare)
                if (tempPiece?.chessman == Chessman.QUEEN && isRowCleanBetween(tempSquare,
                        square) && tempPiece.player == player)
                {
                    resultList.add(tempPiece)
                }
        ...
        for (row in 0..7)
        {
            if (row != square.row)
            {
                val tempSquare = Square(row, square.col)
                val tempPiece = getPieceFrom(tempSquare)
                if (tempPiece?.chessman == Chessman.QUEEN && isColumnCleanBetween(tempSquare,
                        square) && tempPiece.player == player)
                {
                    resultList.add(tempPiece)
                }
        ...
        while (square.col + index < 8 && square.row + index < 8)
        {
            val tempSquare = Square(square.row + index, square.col + index)
            val tempPiece = getPieceFrom(tempSquare)
            if (tempPiece?.chessman == Chessman.QUEEN && tempPiece.player == player && isDiagonalCleanBetween(
                    tempSquare,
                    square))
            {
                resultList.add(tempPiece)
    ...
    {
        if (squareOne.row != squareTwo.row) throw Exception("There are diff rows in args")

        val minCol = min(squareOne.col, squareTwo.col)
        val maxCol = max(squareOne.col, squareTwo.col)

        if (minCol == maxCol || maxCol == minCol + 1) return true
    ...
    {
        if (squareOne.col != squareTwo.col) throw Exception("There are diff columns in args")

        val minRow = min(squareOne.row, squareTwo.row)
        val maxRow = max(squareOne.row, squareTwo.row)

        if (minRow == maxRow || maxRow == minRow + 1) return true
    ...
    fun makeMove(move: MutableList<Square>)
    {
        ...
        for (i in 0 until move.size / 2)
        {
            ...
            {
                deletePieceFromField(move[i * 2 + 1])
            }

            val tempPiece =
                piecesBox.find { it.row == move[i * 2 + 0].row && it.col == move[i * 2 + 0].col }
            piecesBox.removeIf { it.row == move[i * 2 + 0].row && it.col == move[i * 2 + 0].col }
            val piece = tempPiece?.let {
                ChessPiece(move[i * 2 + 1].row,
                    move[i * 2 + 1].col,
                    tempPiece.player,
                    tempPiece.chessman,
                    tempPiece.resID)
    ...
    private fun canKingMove(from: Square, to: Square): Boolean
    {
        if (canQueenMove(from, to))
        {
            ...
            return (deltaCol + deltaRow == 1) || (deltaCol + deltaRow == 2) || (deltaCol == 2 && deltaRow == 0)
        }
        return false
    }
